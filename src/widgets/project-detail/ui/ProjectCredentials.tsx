...
export default function ProjectCredentials({ credentials }: Props) {
  ...
  return (
    <section className="glass-card rounded-xl overflow-hidden">
      <header className="flex items-center gap-2 px-5 py-3 border-b border-border/60 bg-black/20 font-mono text-xs">
      ...
      <div className="divide-y divide-border/40">
        {credentials.map((cred, i) => {
          ...
          return (
            <div key={keyPrefix} className="p-5 font-mono text-sm">
              ...
              {note && (
                <p className="mt-3 text-xs text-muted-foreground font-sans leading-relaxed">
                  {note}
                </p>
              )}
            </div>
      ...
      <footer className="px-5 py-2 border-t border-border/60 bg-black/20 text-[10px] font-mono text-muted-foreground/70">
...
function CredRow({
  ...
  return (
    <div className="flex items-center gap-3 text-emerald-300/90">
      <span className="w-20 shrink-0 text-muted-foreground/60">{label}</span>
